...
def unTarFiles(outputTarFile, output_path):
    ...
    print("File untar Successfully: ",output_path)
    tarFile.close()

def getNativePlatformData(nativePlatformFilePath):
    with open(nativePlatformFilePath, 'r') as file:
        data = json.load(file)
    print("Getting the native platform data")
    return data
...
def insertNativePlatformData(df_np, TimeStamp):
    try:
        ...
        uuid = cur.fetchone()
        print("Inserting native platform Data")
        return uuid
    except Exception as e:
        print(f"\n{'=' * 30}\n{e}\n{'=' * 30}\n")
...
def validate_workload_json(new_filename, workloadFilePath, start_date):
    ...
    try:
        ...
        if any(x in njmon_cols for x in json_cols):
            ...
            updateStatusTbl(new_filename, endTime, process_msg, status, status_percentage, start_date)
            print("Workload data validated")
            workload_response = jsonify({"Message":"File copied to server Successfully!", "status":1})
        else:
            endTime = None
            ...
            status_percentage = "100%"
            updateStatusTbl(new_filename, endTime, process_msg, status, status_percentage, start_date)
            print("Workload data failed to validated")
...
def workload_etl(benchmark_tag, uuid_key, host, tag, workload, iteration, workloadFilePath, is_workload_data, new_filename, start_date):

    endTime = None
    process_msg = "Started ETL Process"
    status = "Running"
    status_percentage = "30%"
    print("ETL process 30%' completed")
    updateStatusTbl(new_filename, endTime, process_msg, status, status_percentage, start_date)
            
    print("args:","host:", host, "workload:", workload, "iteration", iteration, "filename:", workloadFilePath, "is_workload_data:", is_workload_data)
    ...
    for table in table_list:
        df_table = load_with_tags(dict_list_final, uuid_key, benchmark_tag, host, workload, iteration, is_workload_data, tag, table)
...
def getWorkloadReturnVal(host):
    ...
    for table in tableList:
        ...
        tableCount.append(r)
    print("Record updated in all tables")
    return tableCount
...
def convert_to_csv(transformed_array):
    ...
    print("Filtered data get converted to csv")
    return df
